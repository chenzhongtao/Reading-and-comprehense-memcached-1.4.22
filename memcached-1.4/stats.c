...
//stats参考:http://www.cnblogs.com/xianbei/archive/2011/01/02/1921258.html
/*
...
STORED
stats detail dump
PREFIX yang get 0 hit 0 set 1 del 0
END
*/
#define PREFIX_HASH_SIZE 256

//stats参考:http://www.cnblogs.com/xianbei/archive/2011/01/02/1921258.html
//分配了一个256个元素的数组用来存储所有命名空间对象的状态记录,所有命名空间字符串都是经过hash之后分配的,hash之后值相同的命名空间会组成单向链表
static PREFIX_STATS *prefix_stats[PREFIX_HASH_SIZE]; //存储key相关的详细信息 
static int num_prefixes = 0; //prefix_stats hash表中总的key节点个数
...
static PREFIX_STATS *stats_prefix_find(const char *key, const size_t nkey) {
    ...
    for (length = 0; length < nkey && key[length] != '\0'; length++) {
        if (key[length] == settings.prefix_delimiter) { //如果key中包含prefix_delimiter字符，则只计算该字符前面的字符串
            bailout = false;
            break;
        }
    }
    ...
    //如果hash中已经存在该key，则直接返回，否则创建新的节点存到hash中
 ...
 */ //对key进行写操作的相关统计，包括add set replace prepend append命令对key的操作
